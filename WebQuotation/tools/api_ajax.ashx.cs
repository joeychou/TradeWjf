...
namespace WebQuotation.tools
{
    ...
    public class api_ajax : IHttpHandler
    {
        ...
        private void rds_clear(HttpContext context)
        {
            string opt = AppRequest.GetQueryString("opt");
            string key = AppRequest.GetQueryString("key");
            if (opt == "remove")
            {
                RedisHelper.Remove(key);
                context.Response.Write(JsonHelper.ObjectToJSON(new { succeess = true, msg = "清理成功" }));
                return;
            }
            context.Response.Write(JsonHelper.ObjectToJSON(new { succeess = false, msg = "error" }));
            return;
        }
