...
function getKLData() {
    ...
    $.ajax({
        url: api_domain_url + 'tools/stock_line_k.ashx',
        ...
        success: function (data) {
            ...
            if (data.kmins) {
                ...
                $.each(res, function (i, v) {
                    ohlc.push([
                        new Date(convertDate(v.time)).getTime(), // the date
                        v.open, // open
                        v.highest, // high
                        v.lowest, // low
                        v.preClose // close
                    ...
                    tooltip: {
                        ...
                        formatter: function () {
                            ...
                            $.each(data.kmins, function (i, v) {
                                var time = new Date(convertDate(v.time)).getTime()
                                if (me.x == time) {
                                    nowtime = v.time;
                                    ...
                                    high = parseFloat(v.highest);
                                    low = parseFloat(v.lowest);
                                    close = parseFloat(v.preClose);
                                    y = (parseFloat(v.volume) * 0.0001).toFixed(2);
                            ...
                            var tip = '<b>' + convertDate(nowtime) + '</b><br/>';
                            //tip += stockName + "<br/>";
