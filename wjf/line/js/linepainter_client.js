...
linePainter.prototype = {
    ...
    paintArea: function (i, x, y , i1, x1 , y1) {
        ...
        var grad = ctx.createLinearGradient(x,region.y/2 + region.height/2 -15,x1,y1);
				grad.addColorStop(0,'rgba('+parseInt(options.gradientColor.substring(1,3),16)+','+parseInt(options.gradientColor.substring(3,5),16)+','+parseInt(options.gradientColor.substring(5,7),16)+',0.05)');
				grad.addColorStop(1,'rgba('+parseInt(options.gradientColor.substring(1,3),16)+','+parseInt(options.gradientColor.substring(3,5),16)+','+parseInt(options.gradientColor.substring(5,7),16)+',0.35)');
				ctx.fillStyle = grad;
        if (i == 0) {
