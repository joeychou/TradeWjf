...
namespace DBUtility
{
    ...
    public abstract class DbHelperMySQL
    {
        ...
        public static string connectionString = Common.Utils.ConnDecrypt(ConfigurationManager.AppSettings["ConnectionString"].ToString());
        ...
        /// <param name="SQLString">计算查询结果语句</param>
        /// <returns>查询结果（object）</returns>
        public static object GetSingleTo(string SQLString)
        {
            string ConnectionStringTo = Common.Utils.ConnDecrypt(ConfigurationManager.AppSettings["ConnectionStringTo"].ToString());
            using (MySqlConnection connection = new MySqlConnection(ConnectionStringTo))
            {
                using (MySqlCommand cmd = new MySqlCommand(SQLString, connection))
                {
                    try
        ...
        /// 执行存储过程,返回DataSet
        /// </summary>
        /// <param name="proce">存储过程名</param>
        /// <param name="cmdParms">传入参数</param>
        /// <returns></returns>
        public static DataSet ExecuteProceDataSet(string proce, MySqlParameter[] cmdParms)
        {
            DataSet ds = new DataSet();
            try
            {
                ...
                {
                    connection.Open();
                    using (MySqlDataAdapter adapter = new MySqlDataAdapter(proce, connection))
                    {
        ...
        /// </summary>
        /// <param name="storeProceName">存储过程名</param>
        /// <param name="paraValues">传递给存储过程的参数值列表</param>
        /// <returns></returns>
        public static int ExecuteNonQuery(string storeProceName, params object[] paraValues)
        {
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                MySqlCommand command = new MySqlCommand(storeProceName, connection);
                ...
                try
                {
                    ...
                    if (paraValues != null)
                    {
                        if ((command.Parameters.Count - 1) != paraValues.Length)
                        {
                            throw new ArgumentNullException("The number of parameters does not match number of values for stored procedure.");
                        }
                        for (int i = 0; i < paraValues.Length; i++)
                        {
                            command.Parameters[i + 1].Value = (paraValues[i] == null) ? DBNull.Value : paraValues[i];
                        }
                    }
                    connection.Open();
        ...
        public static int ExecuteNonQuery(string cmdText, params MySqlParameter[] cmdParms)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(connectionString))
            {
                PrepareCommand(conn, null, cmd, CommandType.StoredProcedure, cmdText, cmdParms);
                int val = cmd.ExecuteNonQuery();
                cmd.Parameters.Clear();
                return val;
            }
        ...
        public static object ExecuteScalar(string cmdText, params MySqlParameter[] cmdParms)
        {
            ...
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                PrepareCommand(connection, null, cmd, CommandType.StoredProcedure, cmdText, cmdParms);
        ...
        public static void ExecProcOutput(out object[] output, string cmdText, params MySqlParameter[] cmdParms)
        {
            MySqlCommand cmd = new MySqlCommand();
            ...
            {
                PrepareCommand(conn, null, cmd, CommandType.StoredProcedure, cmdText, cmdParms);
                int length = cmdParms.Length;
                output = new object[length];//存储过程中返回值的个数
                try
                {
                    cmd.ExecuteNonQuery();
                }
                ...
                for (int i = 0; i < length; i++)//将存储过程返回的参数值返回到程序中
                {
